...
const STAGES = [
  ...
  { min: 60,  max: 149,  stageIndex: 2, label: "young", message: "키가 쑥쑥 자라는 중.", variants: 1 },
  { min: 150, max: 299,  stageIndex: 3, label: "tree", message: "나무가 제법 자랐다.", variants: 1 },

  // 변이 시작
  { min: 300, max: 599,  stageIndex: 4, label: "mutation I", message: "형태가 변하기 시작한다.", variants: 5 },

  // 강한 변이
  { min: 600, max: 699,  stageIndex: 5, label: "mutation II", message: "기묘한 기운이 돈다.", variants: 5 },

  // 형태 완전 변화
  { min: 700, max: 799,  stageIndex: 6, label: "weird form", message: "나무라기엔 너무 이상하다.", variants: 5 },

  // 유기체적 형태
  { min: 800, max: 899,  stageIndex: 7, label: "entity", message: "넌 뭐가 되고싶은걸까?", variants: 5 },

  // 기하학 + 동물 암시
  { min: 900, max: 999,  stageIndex: 8, label: "unknown being", message: "조금은 알 것 같아.", variants: 5 },

  // 엔딩
  { min: 1000, max: 99999, stageIndex: 9, label: "final", message: "너는 그게 되고 싶었구나. 기특해!", variants: 1 },
];
...
function applyStage(stageIndex, variantIndex) {
  ...
  for (let i = 0; i <= 9; i++) tree.classList.remove(`stage-${i}`);
  tree.classList.add(`stage-${stageIndex}`);

  // remove variant classes
  for (let i = 0; i <= 4; i++) tree.classList.remove(`variant-${i}`);
